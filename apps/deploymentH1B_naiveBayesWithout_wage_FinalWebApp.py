...
loaded_model = pickle.load(open('trained_model_H1B_visa.pkl','rb'))
...
def Visa_prediction(SOC_TITLE_NEW, EMPLOYER_BRANCH, JOB_TITLE_NEW, SOC_CODE_NEW, NAICS_CODE_NEW):
...
def app():
    ...
    #giving a title
    st.subheader('Please provide the information of Employee skills.')
    ...
    if st.button('Employee Request for H1B visa'):
        reviewer = Visa_prediction(SOC_TITLE_NEW, EMPLOYER_BRANCH, JOB_TITLE_NEW, SOC_CODE_NEW, NAICS_CODE_NEW)
